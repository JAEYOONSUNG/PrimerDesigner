#' Download GenBank .gbff File
#'
#' Downloads a GenBank .gbff file for a given accession number from NCBI.
#' Creates a directory to store the downloaded file and retrieves metadata such as organism name and release date.
#'
#' @param dir A character string specifying the directory to save the .gbff file (default: current working directory).
#' @param accession A character string specifying the GenBank accession number (e.g., "GCF_030376745.1").
#' @return A list containing the file path of the downloaded .gbff file, organism name, release date, and sequence directory.
#' @examples
#' \dontrun{
#' metadata <- download_genbank_gbff(dir = getwd(), accession = "GCF_030376745.1")
#' print(metadata)
#' }
#' @export
#'
download_genbank_gbff <- function(dir = getwd(), accession) {
  low_dir <- "seqs_srcdir"
  ...
  gbff_file <- base::file.path(seqs_dir, base::paste0(accession, ".gbff"))
  base::cat("Downloading .gbff to:", gbff_file, "\n")
  base::sink(gbff_file)
  for (id in nuccore_ids) {
    gbff <- rentrez::entrez_fetch(db = "nuccore", id = id, rettype = "gb", retmode = "text")
    base::cat(gbff)
  }
  base::sink()

  if (!base::file.exists(gbff_file)) {
    base::stop("Failed to download .gbff file for accession:", accession)
  }
